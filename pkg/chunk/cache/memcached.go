...
func (cfg *MemcachedConfig) RegisterFlagsWithPrefix(prefix string, f *flag.FlagSet) {
	if prefix != "" {
		prefix += "."
	}

	f.DurationVar(&cfg.Expiration, prefix+"memcached.expiration", 0, "How long keys stay in the memcache.")
	f.IntVar(&cfg.BatchSize, prefix+"memcached.batchsize", 0, "How many keys to fetch in each batch.")
	f.IntVar(&cfg.Parallelism, prefix+"memcached.parallelism", 100, "Maximum active requests to memcache.")
}
