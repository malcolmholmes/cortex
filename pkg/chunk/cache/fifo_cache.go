...
func (cfg *FifoCacheConfig) RegisterFlagsWithPrefix(prefix string, f *flag.FlagSet) {
	if prefix != "" {
		prefix += "."
	}

	f.IntVar(&cfg.Size, prefix+"fifocache.size", 0, "The number of entries to cache.")
	f.DurationVar(&cfg.Validity, prefix+"fifocache.duration", 0, "The expiry duration for the cache.")
}
