...
func (cfg *DiskcacheConfig) RegisterFlagsWithPrefix(prefix string, f *flag.FlagSet) {
	if prefix != "" {
		prefix += prefix + "."
	}

	f.StringVar(&cfg.Path, prefix+"diskcache.path", "/var/run/chunks", "Path to file used to cache chunks.")
	f.IntVar(&cfg.Size, prefix+"diskcache.size", 1024*1024*1024, "Size of file (bytes)")
}
